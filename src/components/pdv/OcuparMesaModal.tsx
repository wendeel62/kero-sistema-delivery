...
export default function OcuparMesaModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-6" onClick={onCancel}>
      <div className="bg-[#1a1a1a] rounded-3xl p-8 w-full max-w-sm border border-[#252830] shadow-2xl animate-fade-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input type="number" min="1" max={mesa.capacidade} value={pessoas} onChange={e => onPessoasChange(Number(e.target.value))} className="w-full bg-[#16181f] border border-[#252830] rounded-xl py-3 px-4 text-sm text-white" />
          </div>
          <div>
            <label className="text-xs font-bold text-gray-400 mb-2 block">Nome do Responsável (opcional)</label>
            <input value={responsavel} onChange={e => onResponsavelChange(e.target.value)} placeholder="Ex: João" className="w-full bg-[#16181f] border border-[#252830] rounded-xl py-3 px-4 text-sm text-white" />
          </div>
        </div>
